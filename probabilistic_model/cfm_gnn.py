...
class PairNorm(nn.Module):
    ...
    def __call__(self, features, rescale_factor=1.0):
        ...
        return (feature_centered / (feature_l2 + eps)) * feature_l2_mean * rescale_factor
...
class GraphNorm(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, counts, deterministic: bool):
        N_total = x.shape[0]
        G = counts.shape[0]
...
def get_node_mlp_updates(mlp_feature_sizes: Sequence[int], dropout_rate: float = 0.0,
    ...
    def update_fn(nodes, sent_attributes, received_attributes, globals, deterministic=True):
...
def get_attention_logit_fn(hidden_size: int, dropout_rate: float = 0.0, name: str = None) -> Callable:
...
class GraphConvNet(nn.Module):
    ...
    @nn.compact
    def __call__(self, graph: jraph.GraphsTuple, deterministic: bool) -> jraph.GraphsTuple:
        ...
        if self.shared_weights:
            node_mlp_shared = MLP(mlp_feature_sizes, dropout_rate=self.dropout_rate, name="update_node_fn_shared")
            edge_mlp_shared = MLP(mlp_feature_sizes, dropout_rate=self.dropout_rate, name="update_edge_fn_shared")
            if self.attention:
        ...
        for step in range(self.message_passing_steps):
            ...
            if self.shared_weights:
                ...
                update_node_fn = get_node_mlp_updates(
                    mlp_feature_sizes, dropout_rate=self.dropout_rate, name=f"update_node_fn_{step}"
                )
                update_edge_fn = get_edge_mlp_updates(
                    mlp_feature_sizes, dropout_rate=self.dropout_rate,
                    relative_updates=self.relative_updates, name=f"update_edge_fn_{step}"
                )
                graph_net = jraph.GraphNetwork(
            ...
            if self.skip_connections:
                ...
                graph = graph._replace(
                    ...
                    edges=(new_graph.edges if (graph.edges is None or not self.edge_skip_connections)
        ...
        # Decode to desired output dim
        decode_node_mlp = MLP([self.hidden_size] * self.num_mlp_layers + [self.output_dim],
                              dropout_rate=self.dropout_rate)
        decoder = jraph.GraphMapFeatures(
            embed_node_fn=functools.partial(decode_node_mlp, deterministic=deterministic)
        )
        return decoder(graph)


# -------------------------
# Time embedding + CFM wrapper
# -------------------------
class FourierTimeEmbedding(nn.Module):
    ...
    def __call__(self, t: jnp.ndarray) -> jnp.ndarray:
        """
        t: shape (batch_nodes, 1) or (batch_nodes,)
        returns: (batch_nodes, emb_dim)
        """
        if t.ndim == 1:
            t = t[:, None]
        half = self.emb_dim // 2
        freqs = jnp.exp(jnp.linspace(jnp.log(1.0), jnp.log(1000.0), half))
        args = 2.0 * jnp.pi * t * freqs[None, :]
...
class CFMGraphModel(nn.Module):
    """
    Conditional Flow Matching model:
      v_theta(x_t, t | graph_context) -> per-node velocity in target space.

    We keep your GraphConvNet backbone, but we feed augmented node features:
      [context_nodes, x_t, time_embedding]
    and output_dim = target_dim (3 for z,vx,vy).
    """
    ...
    def __call__(
        ...
        x_t: jnp.ndarray,          # (batch_nodes, target_dim)
        t: jnp.ndarray,            # (batch_nodes,) or (batch_nodes,1)
        deterministic: bool
    ) -> jnp.ndarray:
        time_emb = FourierTimeEmbedding(self.time_emb_dim)(t)  # (batch_nodes, time_emb_dim)

        # Ensure x_t has correct shape
        if x_t.ndim == 1:
            x_t = x_t[:, None]

        aug_nodes = jnp.concatenate([graph.nodes, x_t, time_emb], axis=-1)

        aug_graph = graph._replace(nodes=aug_nodes)

        out_graph = self.backbone(aug_graph, deterministic=deterministic)
        v = out_graph.nodes

        # Ensure output shape is (batch_nodes, target_dim)
        if v.ndim == 1:
            v = v[:, None]
        return v